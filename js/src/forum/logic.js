...
export function createHowlerPlayer(url, opts = {}) {
  ...
  return {
    ...
    play(startPosition) {
      const pos = id != null ? (howl.seek(id) != null ? howl.seek(id) : 0) : (startPosition != null ? startPosition : 0);
      const soundsBefore = (howl._sounds && howl._sounds.length) || 0;
      ...
      id = howl.play(id != null ? id : undefined);
      const soundsAfter = (howl._sounds && howl._sounds.length) || 0;
    ...
    position() {
      if (id != null) return howl.seek(id) != null ? howl.seek(id) : 0;
      return 0;
    },
    duration() {
  ...
  start() {
    ...
    const tick = () => {
      for (const player of this.activePlayers) {
        if (player.updateProgress) player.updateProgress();
      }
...
function createInlinePlayer(url, progressElement, opts = {}) {
  ...
  player.play = () => {
    ...
    requestAnimationFrame(function () { if (player.updateProgress) player.updateProgress(); });
    return id;
  };
...
export function initGlobalPlayer(app) {
  ...
  log('Global player tracks', { tracksLength: tracks && tracks.length, tracksOk });
  ...
  function playCurrent() {
    isSwitchingTrack = false;
    log('playCurrent()', { hasPlayer: !!player, isPlaying: player && player.playing && player.playing(), currentPosition });
    if (!player) {
      loadTrack(currentIndex, () => {
        if (!player) return;
  ...
  createUI();
  loadTrack(currentIndex, () => {
    log('init load callback', { isPlaying, currentPosition, playerPlaying: player && player.playing && player.playing() });
    if (isPlaying) {
